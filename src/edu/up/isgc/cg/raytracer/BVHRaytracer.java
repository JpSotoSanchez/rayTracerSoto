...
import edu.up.isgc.cg.raytracer.tools.BVHNode;
...
public class BVHRaytracer {
    ...
    public static void main(String[] args) {
        ...
        Scene scene1 = SceneCreator.finalScene01();
        //Scene scene2 = SceneCreator.finalScene02();
        //Scene scene3 = SceneCreator.finalScene03();
        //Scene scene4 = SceneCreator.reflectionDisplay();
        //Scene scene5 = SceneCreator.refractionDisplay();


        BufferedImage image = raytrace(scene1);
        
        File outputImage = new File("testingTime.png");
        try {
    ...
    public static BufferedImage raytrace(Scene scene) {
        ...
        // Build BVH for scene acceleration
        System.out.println("Building BVH...");
        Date bvhStart = new Date();
        BVHNode bvh = new BVHNode(objects);
        Date bvhEnd = new Date();
        double bvhBuildTime = (bvhEnd.getTime() - bvhStart.getTime()) / 1000.0;
        
        System.out.println("BVH built in " + String.format("%.3f", bvhBuildTime) + "s");
        System.out.println("BVH Stats:");
        System.out.println("  - Tree depth: " + bvh.getDepth());
        System.out.println("  - Leaf nodes: " + bvh.countLeaves());
        System.out.println("  - Total objects: " + bvh.countTotalObjects());
        ...
        try {
                ...
                for (int tx = 0; tx < tilesX; tx++) {
                    ...
                    tasks[taskIndex++] = CompletableFuture.runAsync(() -> {
                        renderTile(
                            ...
                            objects, lights, scene, rgbBuffer, completedTiles, bvh
                        );
                    }, forkJoinPool);
                }
            }
            ...
            allTasks.get(100, TimeUnit.HOURS);
    ...
    private static void renderTile(
            ...
            int[] rgbBuffer, AtomicInteger completedTiles, BVHNode bvh) {
        ...
        for (int y = startY; y < endY; y++) {
            for (int x = startX; x < endX; x++) {
                ...
                Intersection intersection = raycastBVH(ray, bvh, null, depthRange);
                
                Color pixelColor = Color.BLACK;
                if (intersection != null) {
                    pixelColor = calculatePixelColor(intersection, scene, lights, objects, ray, 0, bvh);
                }
    ...
    private static Color calculatePixelColor(Intersection intersection, Scene scene, List<Light> lights, 
                                          List<Object3D> objects, Ray ray, int depth, BVHNode bvh) {
        Color pixelColor = Color.BLACK;
        Color objColor = intersection.getObject().getMaterial().getColor();
        
        for (Light light : lights) {
            Color diffuseColor = calculateLightContribution(light, intersection, objects, objColor, bvh);
            pixelColor = addColor(pixelColor, diffuseColor);
            
            Color specularColor = calculateSpecularLight(light, intersection, scene, objects, bvh);
        ...
        if (reflectionFactor > 0 && depth < scene.getMaxReflectionDepth()) {
            Color reflectedColor = calculateReflection(ray, intersection, scene, objects, depth + 1, bvh);
        ...
        if (refractionIndex > 1.0 && transparencyFactor > 0 && depth < scene.getMaxReflectionDepth()) {
            Color refractedColor = calculateRefraction(ray, intersection, scene, objects, depth + 1, bvh);
    ...
    private static Color calculateReflection(Ray ray, Intersection intersection, Scene scene, 
                                           List<Object3D> objects, int depth, BVHNode bvh) {
        ...
        Intersection reflectedIntersection = raycastBVH(reflectedRay, bvh, intersection.getObject(), null);
        
        if (reflectedIntersection == null) {
            return Color.BLACK;
        }
        
        return calculatePixelColor(reflectedIntersection, scene, scene.getLights(), objects, reflectedRay, depth, bvh);
    }

    private static Color calculateRefraction(Ray ray, Intersection intersection, Scene scene, 
                                           List<Object3D> objects, int depth, BVHNode bvh) {
        ...
        if (k < 0.0) {
            return calculateReflection(ray, intersection, scene, objects, depth, bvh);
        }
        ...
        Intersection nextIntersection = raycastBVH(refractedRay, bvh, entering ? null : currentObject, null);
        ...
        if (entering && nextIntersection.getObject().equals(currentObject)) {
            ...
            Color exitColor = calculateRefraction(refractedRay, nextIntersection, scene, objects, depth + 1, bvh);
            
            return applyMaterialAbsorption(exitColor, materialColor, absorption);
        } else {
            return calculatePixelColor(nextIntersection, scene, scene.getLights(), objects, refractedRay, depth + 1, bvh);
        }
    }
    ...
    private static Color calculateLightContribution(Light light, Intersection intersection, 
                                                  List<Object3D> objects, Color objColor, BVHNode bvh) {
        if (isPointInShadow(light, intersection, objects, bvh)) {
            return Color.BLACK;
        }
    ...
    private static Color calculateSpecularLight(Light light, Intersection intersection, Scene scene, 
                                              List<Object3D> objects, BVHNode bvh) {
        if (isPointInShadow(light, intersection, objects, bvh)) {
            return Color.BLACK;
        }
    ...
    private static boolean isPointInShadow(Light light, Intersection intersection, List<Object3D> objects, BVHNode bvh) {
        ...
        Intersection shadowIntersection = shadowCastBVH(lightRay, bvh, null);
    ...
    // NEW BVH-accelerated raycast methods
    public static Intersection raycastBVH(Ray ray, BVHNode bvh, Object3D caster, double[] clippingPlanes) {
        double tMin = EPSILON;
        double tMax = Double.POSITIVE_INFINITY;
        
        if (clippingPlanes != null) {
            tMax = 1000.0; // Large but finite value for clipping
        }
        
        Intersection intersection;
        
        // Use the enhanced intersect method that can exclude objects
        if (caster != null) {
            intersection = bvh.intersectExcluding(ray, tMin, tMax, caster);
        } else {
            intersection = bvh.intersect(ray, tMin, tMax);
        }
        
        // Apply clipping planes if specified
        if (intersection != null && clippingPlanes != null) {
            double intersectionZ = intersection.getPosition().getZ();
            if (intersectionZ < clippingPlanes[0] || intersectionZ > clippingPlanes[1]) {
                return null;
            }
        }
    ...
    public static Intersection shadowCastBVH(Ray ray, BVHNode bvh, Object3D caster) {
        double tMin = EPSILON;
        double tMax = Double.POSITIVE_INFINITY;
        
        // Use the enhanced intersect method that can exclude objects
        if (caster != null) {
            return bvh.intersectExcluding(ray, tMin, tMax, caster);
        } else {
